...
var _ = Describe("configurator", func() {
	...
	It("should return sensible objects from ReadConfigFile", func() {
		...
		if printme {
			for _, d := range c.DomainConfigs {
				...
				fmt.Println(d.Debug)
				for i, g := range d.GoogleEmailDomains {
					fmt.Printf("g:%d: %s\n", i, g.Domain)
					fmt.Printf("g:%d: %s\n", i, g.OAuthClientID)
				...
				fmt.Println(d.SkipTLSValidation)
			}
		}
	})
	...
	It("should return GoogleEmailDomain from DomainConfig", func() {
		...
		ged := dc.GetGoogleEmailDomain("second.example.com")
		Expect(ged.OAuthClientID).To(Equal("def"))
		Expect(ged.OAuthClientSecret).To(Equal("456"))
	})
})
